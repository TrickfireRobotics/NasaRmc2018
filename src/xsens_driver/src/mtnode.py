#!/usr/bin/env python
#driver by xsens at on:https://github.com/xsens/xsens_mti_ros_node 
#driver modified and maintained for trickfire robotics by Collin
...
class XSensDriver(object):
    ...
    def spin_once(self):
        ...
        if acc_data:
            rospy.logdebug("acc available & not gyr %d & not orientation %d",[not gyr_data , not orient_data])
        ...
        if gyr_data:
            rospy.logdebug("gyro available & not acc %d & not orientation %d",[not acc_data , not orient_data])
        ...
        if orient_data:
            rospy.logdebug("orientation available & not acc %d & not gyro %d",[not acc_data , not gyr_data])
